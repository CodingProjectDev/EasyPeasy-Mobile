...
function apiBaseUrl() {
  const value =
    process.env.EXPO_PUBLIC_STORE_API_URL?.trim();

  if (!value) {
    throw new Error(
      'EXPO_PUBLIC_STORE_API_URL is missing from .env.',
    );
  }
...
export async function uploadPaymentProof(
  ...
  if (!path) {
    throw new Error(
      'Payment proof uploaded without a storage path.',
    );
  }
...
export async function updateProductNotification(
  ...
  const response = await fetch(
    `${apiBaseUrl()}/api/customer/product-notification`,
    {
      ...
      body: JSON.stringify({ productId, action, operation }),
    },
  );
  if (!response.ok) {
    const payload = (await responseJson(response)) as Record<string, unknown>;
...
export async function uploadSellPhoto(
  ...
  if (Platform.OS === 'web') {
    const fileResponse = await fetch(file.uri);
    form.append('file', await fileResponse.blob(), file.name);
  } else {
  ...
  const payload = (await responseJson(response)) as Record<string, unknown>;
  if (!response.ok) throw apiError(payload, 'Could not upload item photo.');
...
export async function submitSellRequest(
  ...
  if (!response.ok) throw apiError(result, 'Could not submit your item.');
  return result;
}
...
export async function loadSellRequests(accessToken: string) {
  ...
  const result = (await responseJson(response)) as Record<string, unknown>;
  if (!response.ok) throw apiError(result, 'Could not load selling items.');
...
export async function sendContactMessage(payload: {
  ...
  const result = (await responseJson(response)) as Record<string, unknown>;
  if (!response.ok) throw apiError(result, 'Could not send your message.');
  return result;
}
